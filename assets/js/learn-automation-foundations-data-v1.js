(() => {
  window.ShieldioAutomationFoundations = {
    'automatizace-01-zaklady-cislicove-techniky': {
      ...
      chapters: [
        ['Od čísla k logickému obvodu', 'Číslicová technika popisuje informaci diskrétními stavy. Jeden bit nabývá hodnoty 0 nebo 1, více bitů tvoří slovo. U maturity propoj tři části: zápis a převod čísel, popis logické funkce a její realizaci hradly.'],
        ['Poziční číselné soustavy', 'V soustavě o základu z smí číslice nabývat hodnot 0 až z−1. Hodnota zápisu je součet číslic násobených mocninami základu. Běžně používáme desítkovou, dvojkovou, osmičkovou a šestnáctkovou soustavu; dvanáctková je příkladem jiné možné soustavy. Dolní index vždy uvádí základ.'],
        ['Desítková → dvojková', 'Celou část čísla opakovaně děl dvěma. V každém řádku zapiš celočíselný podíl a zbytek 0 nebo 1. Jakmile podíl dosáhne nuly, přečti zbytky zdola nahoru. Pro 13 vyjdou zbytky 1, 0, 1, 1, tedy 13₁₀ = 1101₂.'],
        ['Desítková → libovolný základ', 'Stejný algoritmus funguje pro základ z: dělíš číslem z a zapisuješ zbytky. Pro šestnáctkovou soustavu nahrazují hodnoty 10 až 15 písmena A až F. Příklad: 254 : 16 = 15 zbytek 14 (E), 15 : 16 = 0 zbytek 15 (F), proto 254₁₀ = FE₁₆.'],
        ['Dvojková → desítková', 'Pozice zprava mají váhy 2⁰, 2¹, 2²… Sečti pouze váhy míst, na kterých je jednička: 11010₂ = 1·2⁴ + 1·2³ + 0·2² + 1·2¹ + 0·2⁰ = 26₁₀. Stejně převedeš číslo z libovolné soustavy, jen místo dvojky použiješ její základ.'],
        ['Převodník a kontrola postupu', 'Interaktivní převodník ověří výsledek, ale u maturity musíš umět vysvětlit postup. Při převodu z desítkové soustavy ukáže opakované dělení; při převodu do desítkové rozepíše poziční součet. Nepoužívej výsledek kalkulačky bez kontroly povolených číslic.'],
        ['Logické úrovně v obvodu', 'Logická 0 a 1 jsou intervaly napětí, ne dvě univerzální přesné hodnoty. Datasheet stanoví vstupní meze VIL a VIH, výstupní meze VOL a VOH, proudovou zatížitelnost a napájení. Rozdíl zaručené výstupní a vstupní meze tvoří šumovou rezervu; nezapojený vstup nesmí plavat.'],
        ['Hradla a pravdivostní tabulka', 'AND je 1 pouze pro samé jedničky, OR při alespoň jedné jedničce, NOT stav neguje a XOR je 1 při rozdílných vstupech. NAND a NOR jsou univerzální hradla. Pravdivostní tabulka musí obsahovat všech 2ⁿ kombinací n vstupů.'],
        ['Booleova algebra – základní pravidla', 'Značíme AND jako A·B, OR jako A+B a negaci pruhem nebo ¬A. Platí A+A=A, A·A=A, A+¬A=1, A·¬A=0, A+0=A, A·1=A a dvojitá negace ¬¬A=A. Úprava je správná jen tehdy, když zachová výstup pro všechny kombinace vstupů.'],
        ['De Morganovy věty', 'Negace součtu se změní na součin negovaných proměnných: ¬(A+B)=¬A·¬B. Negace součinu se změní na součet negovaných proměnných: ¬(A·B)=¬A+¬B. Při přechodu přes negaci se tedy zamění AND ↔ OR a neguje každý operand.'],
        ['Úplné tvary funkce', 'Z řádků tabulky s Q=1 vznikají mintermy a jejich součet dává úplnou disjunktivní normální formu. Z řádků s Q=0 lze obdobně sestavit součin maxtermů. Tyto tvary jsou jednoznačným začátkem návrhu, ale často nejsou nejúspornější realizací.'],
        ['Karnaughova mapa', 'Karnaughova mapa řadí sousední kombinace v Grayově pořadí 00, 01, 11, 10, aby se sousedé lišili právě jedním bitem. Jedničky se slučují do co největších obdélníků o velikosti 1, 2, 4, 8… Okraje mapy spolu sousedí a skupiny se smějí překrývat. Stav X (don’t care) lze využít, jen když skupinu výhodně zvětší.'],
        ['Vyřešená Karnaughova mapa', 'Pro Q=1 v kombinacích xyz = 000, 010 a 100 vytvoří sloupec yz=00 člen ¬y·¬z. Přes levý a pravý okraj mapy spojíme na řádku x=0 buňky yz=00 a 10, čímž vznikne ¬x·¬z. Výsledek je Q=¬y·¬z+¬x·¬z=¬z·(¬x+¬y).'],
        ['Realizace pouze hradly NAND', 'Součet součinů převedeme pomocí dvojité negace na NAND–NAND tvar: Q = ¬(¬(¬y·¬z) · ¬(¬x·¬z)). Negace vstupů vytvoříme spojením obou vstupů hradla NAND. Dvě hradla vytvoří negované součiny a poslední NAND provede jejich součet podle De Morganovy věty.'],
        ['Kombinační bloky', 'Kombinační obvod nemá paměť; výstup závisí jen na současných vstupech. Půlsčítačka má S=A⊕B a C=A·B, úplná sčítačka přidává Cin. Multiplexor vybírá jeden vstup, dekodér aktivuje výstup podle kódu, komparátor porovnává čísla. Reálný výstup se změní až po době průchodu hradly.'],
        ['Maturitní odpověď v 15 minutách', 'Začni definicí bitu a poziční soustavy, předveď oba směry převodu, vysvětli fyzické logické úrovně a hradla. Pak sestav tabulku, zapiš funkci, minimalizuj ji Karnaughovou mapou a ukaž realizaci NAND. Závěr doplň konkrétním kombinačním obvodem a kontrolou výsledku.']
      ],
      lab: 'logic'
    },
