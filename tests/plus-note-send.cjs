...
const formSource = `gi42._gclass="OspravedlnenkaDlg";gi42.myphpfile="/gcall";gi42._gparam="gpid=1234&gsh=FAKE";ASC.jscFieldsParams(gi42,${JSON.stringify(fields)});gi42.ASC_action('ok',params);var gi88_c = new ASC.ui.ComboBox(null,"",[{"value":"7"}],{});gi42.periodfrom=gi88;var gi99_c = new ASC.ui.ComboBox(null,"",[{"value":"9"}],{});gi42.periodto=gi99;`;
...
const attendance = records => `ASC.requireAsync('/dashboard/dochadzka.js#initZiak').then(function(f){return f(null,null,${JSON.stringify({students:{TEST_STUDENT:{'2026-09-17':records}}})});});`;
...
function harness(mode='success') {
  ...
  ctx.fetch = async (url,opts) => {
    ...
    if (path === '/dashboard/eb.php') {
      ...
      return response(url,attendance(rows));
    }
    if (path === '/gcall') {
      writes++;
      const b=new URLSearchParams(opts.body);
      assert.equal(b.get('action'),'ok');assert.equal(b.get('note'),note.reason);assert.equal(b.get('periodfrom'),'7');assert.equal(b.get('periodto'),'9');assert.equal(b.get('remove_menu_evidence'),'0');assert.equal(b.get('_LJSL'),'4096');
